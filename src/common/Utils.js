...
export class Utils {
  ...
  static renderPage(page, data, appSettings) {
    ...
    ReactDOM.render(
      <App page={page} data={data} appSettings={Utils.appSettings} />,
      domElement
    );
  }
  ...
  static renderGoToSetup(nextAction) {
    return (
      <p>
        {"Kindly go to "}
        <Link onClick={() => Utils.renderPage("setup")}>
          <b>Setup / OMNA</b>
        </Link>
        {" and click "}
        {nextAction}
        {" to the application and billing."}
      </p>
    );
  }
  ...
  static renderGoToUninstall() {
    return (
      <p>
        ...
        <Link onClick={this.handleUninstall}>
          <b>Shopify Dashboard Apps</b>
        </Link>{" "}
        and click the trash.
      </p>
    );
  ...
  static renderLoading(size, title) {
    if (!title && size && size.match(/^large|small$/))
      return <Spinner size={size} color="teal" />;
    ...
    return (
      <Card sectioned title={title || "Loading..."}>
  ...
  static renderExternalLink(title, url, onClick = null) {
    return (
      <Link url={url} external={true} onClick={onClick}>
        {title}
      </Link>
    );
  }
  ...
  static renderNotifications(notifications) {
    if (notifications) {
      return notifications.map((item, idx) => (
        <Card key={idx}>
          {Utils.renderNotification(null, item.message, item.status)}
        </Card>
      ));
    }
  }
  ...
  static get productItems() {
    window.productItems = window.productItems ||
      Utils.getSessionItem("products-items") || {
        items: [],
        count: 0,
        page: 0,
        pages: 0,
  ...
  static getProductIndex(product) {
    return Utils.productItems.items.findIndex(
      item => item.ecommerce_id === product.ecommerce_id
    );
  }
  ...
  static parseResponseError(response) {
    if (response.responseJSON)
      return response.responseJSON.error || response.responseJSON;
  ...
  static variants(product, includeDefault) {
    return includeDefault
      ? product.variants
      : product.variants.filter(v => v.title != "Default Title");
  }
  ...
  static images(item) {
    ...
    var imgs = (images || []).map(img => {
      return {
        original: img.src,
        small: img.src.replace(/\.([^\.]+$)/, "_small.$1")
      };
    });
      ...
      scope.xhr = $.getJSON({
        ...
        .fail(response => {
          const msg =
            ...
            channel +
            " categories. " +
            Utils.parseResponseError(response);
  ...
  static setPropertiesDefinition(channel, category, value) {
...

    {
      ...
      if (keys.length > 5) {
        ...
        keys.forEach(
          k2 => (k1 = pds[k1].accessAt > pds[k2].accessAt ? k2 : k1)
        );
  ...
  static loadPropertiesDefinition(channel, categoryId, scope) {
    ...
    if (!item) {
      ...
      Utils.waitResponse(waitingId, response =>
  ...
  static renderPropertiesGroup(group, gIdx, item, store, renderPropertyField) {
    ...
    context = items.map((item, iIdx) => (
      <FormLayout.Group key={prefixId + iIdx}>
        {group.map((def, pIdx) =>
          renderPropertyField(prefixId + iIdx + "_" + pIdx, def, item)
        )}
      </FormLayout.Group>
    ));

    return title ? (
      <Card sectioned title={title} key={gIdx}>
        {context}
      </Card>
    ) : (
      context
  ...
  static get orderItems() {
    window.orderItems = window.orderItems ||
      Utils.getSessionItem("order-items") || {
        items: [],
        count: 0,
        page: 0,
        pages: 0,
  ...
  static get appSlug() {
    ...
    return (
      urlParams.get("appSlug") ||
      (Utils.isLocal ? "omna-dev" : Utils.appDomainName)
    );
  }
  ...
  static loadSettings(data) {
    let ati = Cookies.get("_ati"),
      ...
      serverDomain = urlParams.has("serverDomain")
        ? urlParams.get("serverDomain")
        : "cenit.io";
    ...
    return new Promise((resolve, reject) => {
      $.getJSON({
        ...
        .done(response => {
          Utils.isLocal &&
            urlParams.has("cache") &&
            Utils.setSessionItem("omna-settings", response.settings);
